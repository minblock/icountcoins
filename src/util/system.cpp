...
const char * const BITCOIN_CONF_FILENAME = "icountcoins.conf";
...
#else
    const char* pszModule = "icountcoins";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ICountCoins";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ICountCoins";
#else
    // Unix
    return pathRet / ".icountcoins";
#endif
#endif
}
